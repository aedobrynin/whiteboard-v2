...
from ..pivot_table_view import show_axis, draw_table, get_attr_from_position
...
def _predicate_from_root_to_add_attribute(
    ...
    return global_dependencies.menu.current_state == SHOW_TABLE_MENU_ENTRY_NAME
...
def _handle_event(
    ...
    if event.type != tkinter.EventType.Deactivate or not window or not window.saved:
        return
    table, x_list, x_options, y_list, y_options, width, height = (
        draw_table(global_dependencies, window.get_vals())
    )
    table.canvas.bind(
        '<B1-Motion>', lambda e: move_obj_start(state_ctx, e, state_ctx[_TABLE])
    )
    table.canvas.bind(
        '<ButtonRelease-1>', lambda e: moving_stop(
            global_dependencies, state_ctx, e, state_ctx[_TABLE]
        )
    )
...
def moving_stop(
    ...
    attributes = get_attr_from_position(position, state_ctx[_X_LIST], state_ctx[_X_OPTIONS],
                                        state_ctx[_Y_LIST],
                                        state_ctx[_Y_OPTIONS], state_ctx[_WIDTH],
                                        state_ctx[_HEIGHT])
...
def _predicate_from_add_attribute_to_root(
    ...
    return event.type == tkinter.EventType.Property
