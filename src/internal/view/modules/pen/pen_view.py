...
class PenObject(ViewObject):
    def __init__(
        ...
        self._line_id = dependencies.canvas.create_line(
            self.get_canvas_points_from_repo_object(dependencies),
            width=obj.width,
            fill=obj.color,
    ...
    def get_canvas_points_from_repo_object(
    ...
    def _subscribe_to_change_points(
        self, dependencies: internal.view.dependencies.Dependencies
    ):
        dependencies.pub_sub_broker.subscribe(
            VIEW_OBJECT_ID,
            ...
            lambda publisher, event, repo: dependencies.canvas.coords(
                self.line_id, self.get_canvas_points_from_repo_object(dependencies)
            )
        )

    def _subscribe_to_change_move(
        self, dependencies: internal.view.dependencies.Dependencies
    ):
        dependencies.pub_sub_broker.subscribe(
            VIEW_OBJECT_ID,
