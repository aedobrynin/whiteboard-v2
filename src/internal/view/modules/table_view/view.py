...
def all_attrs(
    ...
    for obj in dependencies.repo.get_all():
        ...
        attributes = attributes.union(set(list(card.attributes.keys())))
...
def get_options(
    ...
    for obj in dependencies.repo.get_all():
        if not isinstance(obj, internal.objects.interfaces.IBoardObjectCard):
            continue
        card: internal.objects.interfaces.IBoardObjectCard = obj
        options.add(card.attributes.get(name, None))
    return list(options)
...
def draw_table(
    ...
    column_lines = [w.canvas.create_line(start_x + i * width, start_y,
                                         start_x + i * width,
                                         start_y + height * row_num) for i in range(0, col_num + 1)]
    row_lines = [w.canvas.create_line(start_x, start_y + i * height,
                                      start_x + width * col_num,
                                      start_y + i * height) for i in range(0, row_num + 1)]
    ...
    for i, atr in enumerate(x_list):
        ...
        for j, opt in enumerate(x_options[atr]):
            [w.canvas.create_text(start_x + (interval * (opt_n * m + j) + len(y_list) + 1 / 2) * width,
    ...
    for i, atr in enumerate(y_list):
        ...
        for j, opt in enumerate(y_options[atr]):
            [w.canvas.create_text(start_x + (i + 1 / 2) * width,
                                  start_y + (interval * (opt_n * m + j) + len(x_list) + 1 / 2) * height,
    ...
    for obj in dependencies.repo.get_all():
        ...
        reps_x = 1
        for x in x_list:
            val = obj.attributes.get(x, 'default')
            if val != 'default':
        ...
        reps_y = 1
        for y in y_list:
            val = obj.attributes.get(y, 'default')
            if val != 'default':
        ...
        create_card_object(w.canvas, card, ((obj_coords_x[card.id] + len(y_list) + .5) * width + start_x,
