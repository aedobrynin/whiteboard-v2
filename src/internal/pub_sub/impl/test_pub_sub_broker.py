...
def _callback_mock():
    class Mock:
        calls: list[tuple[PublisherId, Event]]

        def __init__(self):
            self.calls = []

        def __call__(self, publisher: PublisherId, event: Event):
            self.calls.append((publisher, event))
...
def test_pub_sub_broker_processing(callback_mock):
    ...
    broker.process_published()
    assert subscriber_callback.calls == [(publisher_id, event)]
